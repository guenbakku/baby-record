...
class Flysystem
{
    ...
    public static function getAdapter(string $uses = null)
    {
        ...
        $config = Configure::read('Storage.flysystem.'.$uses);
    ...
    {
        $uses = Configure::read('Storage.flysystem.uses');
    ...
    {
        if (empty(Hash::get($config, 'auth.credentials.key'))) {
            $config = Hash::remove($config, 'auth.credentials');
        }
        $client = new \Aws\S3\S3Client(Hash::get($config, 'auth'));
